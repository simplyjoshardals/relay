 ...
 *  problem from "the server said no". `resource` defaults to "ticket"
 *  so the existing tickets call sites (TicketsView) don't need updating;
 *  Milestone 4's ServicesView passes "service" explicitly. */
export function failureMessage(
  ...
  if (typeof navigator !== "undefined" && !navigator.onLine) {
    return `You're offline, so the ${resource} wasn't ${action}. Reconnect and try again.`;
  }
  return `Couldn't ${action === "created" ? "create" : "update"} the ${resource}. Try again.`;
}
